...
public class JWTAuthorizationFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        ...
        try{
            ...
            //Pesquiso primeiro por método devido a ter precedência
            ...
            String usuario = containerRequestContext.getSecurityContext().getUserPrincipal().getName();
            throw new UsuarioNaoAutorizadoException("O usuário "+usuario+ " não tem autorização para executar essa funcionalidade. ");
        }
    ...
    private boolean verificarPermissoesDaClasse(SecurityContext securityContext) throws Exception {
        ...
        List<String> permissoesDaClasse = recuperarPermissoesDaClasse(classeDoRecurso);
    ...
    private boolean verificarPermissoesDoMetodo(SecurityContext securityContext) throws Exception {
        ...
        List<String> permissoesDoMetodo = recuperarPermissoesDoMetodo(metodoDoRecurso);
    ...
    private List<String> recuperarPermissoesDaClasse(Class<?> elementoAnotado) {
    ...
    private List<String> recuperarPermissoesDoMetodo(Method elementoAnotado) {
        if (elementoAnotado.isAnnotationPresent(RolesAllowed.class)) {
            RolesAllowed regrasPermitidas = elementoAnotado.getAnnotation(RolesAllowed.class);
            if (regrasPermitidas == null) {
                return new ArrayList<String>();
